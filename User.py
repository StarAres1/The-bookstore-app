...
class User:
    ...
    @staticmethod
    def validate(surname, name, last, birth, new_window, scrollable_frame, root):
        data = [surname.get(), name.get(), last.get(), birth.get()]
    ...
    def add_new_row(root, scrollable_frame):
        ...
        button = ttk.Button(new_window, text="Добавить",
                            command=lambda: User.validate(entry_surname, entry_name, entry_last, entry_birth, new_window, scrollable_frame, root))
    ...
    def sql_add(data, new_window, scrollable_frame, root):
        try:
            query = f"INSERT INTO Пользователи (Имя, Фамилия, Отчество, [Дата рождения]) VALUES (?, ?, ?, ?)"
